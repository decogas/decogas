...
(function () {
  ...
  var cardHTML = function (p, i) {
    var ficha = "producto/" + encodeURIComponent(p.slug);
  ...
  var buildHTML = function (filter) {
    if (!PRODUCTS.length) {
      return '<div class="no-results">Estamos preparando el catálogo de esta sección. ' +
        ...
        'o <a href="./#contacto" style="color:var(--flame-deep); font-weight:600;">pide información</a> y te asesoramos con el equipo que necesitas.</div>';
    }
    var POOL = PRODUCTS.filter(matchesQuery).filter(matchesType).filter(matchesPower);
    if (!POOL.length) {
      return '<div class="no-results">No hay resultados para tu búsqueda. Prueba con otro nombre o marca, o <a href="./#contacto" style="color:var(--flame-deep); font-weight:600;">pídenos información</a>.</div>';
    }
    if (filter === "brand") {
      var brands = [];
